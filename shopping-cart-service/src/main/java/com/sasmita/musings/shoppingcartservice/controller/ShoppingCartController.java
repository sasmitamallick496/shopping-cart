...
public class ShoppingCartController {
	...
	public ResponseEntity<Object> addItem(@PathVariable("cartId") UUID cartId,
	...
	public ResponseEntity<String> submitOrder(@RequestBody ShoppingCartRequest request) throws Throwable{
		SubmitOrderContext context = new SubmitOrderContext();
		context.setRequest(request);
		getExecutor().execute(context);
		request.setStatus(SHOPPING_CART_STATUS_SUBMITTED);
		getCartService().submitCart(request);
		return new ResponseEntity<String>("Order Submitted Succesfully",HttpStatus.OK);
	}
